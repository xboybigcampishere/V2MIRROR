...
function InsertWord( data, word, msg )
	if ( not is_momod(msg) ) then
		send_large_msg ( get_receiver(msg) , "@" .. msg.from.username .. "only admin" );
		return
	end
	...
	if ( TTable ) then
		...
		send_large_msg ( get_receiver(msg) , "word " .. word .. " added to filterlist" );
...
function RemoveWord( data, index, msg )
	...
	if ( TTable ) then
		...
		send_large_msg ( get_receiver(msg) , "word " .. tostring(TTable[index]) .. " delete to filterlist" );
		table.remove(TTable, index)
		save_data(_config.moderation.data, data);
	else
		print("Bot cant add word to list!!!")
	end
end
